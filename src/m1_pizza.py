...
def run_test_draw_points_on_circle():
    ...
    # Test 4
    title = 'Test 4:  7 blue dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(300, 300), 50)
    draw_points_on_circle(window, circle, 7, 'blue')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    circles = generate_points_on_circle(circle, number_of_points)
    for k in range(len(circles)):
        NewCircle = rg.Circle(circles[k], 10)
...
def run_test_pizza():
    ...
    # Test 4
    title = 'PIZZA test 4:  20 black slices on yellow circle'
    window = rg.RoseWindow(520, 400, title)

    circle = rg.Circle(rg.Point(125, 125), 50)
    circle.fill_color = 'yellow'
    pizza(window, circle, 20, 'black', 5)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    title = 'POLYGON tests 3:  6 with medium red lines.'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(350, 200), 100)
    circle.outline_thickness = 5
    polygon(window, circle, 6, 'red', 10)
    window.close_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    # Test 3 (on another window):
    title = 'FANCY POLYGON test 4:  5 lime green lines on black circle, hops = 7.'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'black'
    fancy_polygon(window, circle, 15, 7, 'lime green', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(len(points)):
        if k + hops_to_next_point < len(points):
            ...
            line = rg.Line(points[k], points[k - (len(points)-hops_to_next_point)])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
    window.render()
